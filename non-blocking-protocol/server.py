...
def updatewrite(name_,content_,uuid_,myport):
...
class ServerService(spb2_grpc.ServerServicer):
    ...
    def PrimaryWrite(self, request, context):
        ...
        if (mes == 'SUCCESS'):
            my_thread = threading.Thread(target= updatewrite, args=(name_,content_,uuid_,myport))
            my_thread.start()
            result = {'message': mes}
            return spb2.Message(**result)
        elif(mes == "FAIL DF"):
    ...
    def PrimaryDelete(self, request, context):
        ...
        if (mes == 'SUCCESS'):
            my_thread = threading.Thread(target= updatedelete, args=(uuid_,port))
            my_thread.start()
            result = {'message': mes}
            return spb2.Message(**result)
        elif(mes == "AD"):
    ...
    def WriteFile(self, request, context):
        ...
        if ( res.message == 'SUCCESS'):
            result = {'status':'SUCCESS','uuid_':uuid_,'version':str(datetime.now())}
